				  ...
				  const char *buf, size_t count)
{
	...
	ret = flashProcedure_from_ums(path, mode[0], mode[1]);
					    ...
					    unsigned char *event)
{
	...
	input_sync(info->input_dev);
	dev_dbg(info->dev,
		"%s  %s :  Event 0x%02x - ID[%d], (x, y) = (%3d, %3d) type = %d\n",
...
static irqreturn_t fts_event_handler(int irq, void *ts_info)
{
	...
	if (error != OK) {
		logError(1, "Error (%d) while reading from FIFO in fts_event_handler",
				error);
	} else {
